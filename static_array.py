...
class StaticArray:
    ...
    def set(self, index_: int, value) -> None:
        """
        ...
        if index_ < 0 or index_ >= self.length():
            raise StaticArrayException('Index out of bounds')
        self._data[index_] = value

    def __getitem__(self, index_: int):
        """Enable bracketed indexing."""
        return self.get(index_)

    def __setitem__(self, index_: int, value: object) -> None:
        """Enable bracketed indexing."""
        self.set(index_, value)
